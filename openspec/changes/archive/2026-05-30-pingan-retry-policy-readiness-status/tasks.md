...
- [x] 1.1 Add OpenSpec proposal, design, and delta specs for read-only PingAn retry policy status.
- [x] 1.2 Validate the active OpenSpec change before implementation.

## 2. Red Tests

- [x] 2.1 Add a focused failing test for `dialog_readiness` retry policy status evidence.
- [x] 2.2 Confirm the focused test fails because `retry_policy_status` is missing.

## 3. Implementation

- [x] 3.1 Add read-only retry policy status to PingAn dialog lifecycle status.
