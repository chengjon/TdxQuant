# catalog plan step source name count rollup design

## Design

The catalog plan summary builder already computes `step_source_name_counts` from selected resolved bundle steps and exposes `step_source_name_key_count` in compact rollups. This change copies the existing count map into:
...
- `plan_summary.step_source_name_counts` mirrors `selected_step_summary.step_source_name_counts`.
