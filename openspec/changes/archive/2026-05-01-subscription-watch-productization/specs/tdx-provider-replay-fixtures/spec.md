...
- **AND** the fixture catalog MUST include representative `subscription-watch` run artifacts for canonical events, status, summary, and manifest payloads
...
- **AND** `subscription-watch` status, summary, and manifest fixtures MUST preserve the stabilized run artifact contract fields
...
- **AND** `subscription-watch` event fixtures MUST preserve the canonical event-row run metadata fields
