...
- 对交易执行等桌面环节，当前只保留 TongDaXin 的探测/归因资产；未来真实交易执行主线改由 `PingAN + HID` 承担
- 对 WSL 侧，统一暴露一个稳定调用面
...
- 在交易链路上保留现有 Win32/UIA 探测资产和 HID 基线命令，但不再把通达信真实交易闭环作为当前阶段目标；后续交易执行主线转向 `PingAN + HID`。
...
### 4. 通达信交易自动化仅保留探测与 HID 基线，不再继续追真实交易闭环

现有验证和最新方向已经表明：

- Win32/UIA 适合探测、定位、读回、按钮触发和弹窗抓取
- HID 适合补齐证券代码输入
- 但当前 `TongDaXin V7.73` 环境下交易权限不可用，因此这条线不再继续作为当前阶段交付目标

因此这里的交易 capability 只保留 bridge 健康检查、窗口探测、HID 基线和历史验证资产，不再继续追求通达信真实交易闭环；后续如需真实交易执行，项目方向改为 `PingAN + HID`。
