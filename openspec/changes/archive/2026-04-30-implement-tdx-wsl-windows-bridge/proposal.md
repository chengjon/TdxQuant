...
- 保留通达信交易自动化 bridge 的最小探测与 HID 基线能力，但不再继续把“通达信真实交易闭环”作为当前阶段交付目标；后续交易执行主线改由 `PingAN + HID` 承担。
...
- `tdx-trading-hid-bridge`: 暴露通达信交易探测与 HID 基线能力，供桌面状态归因与历史验证使用；当前阶段不再继续推进真实交易闭环。
