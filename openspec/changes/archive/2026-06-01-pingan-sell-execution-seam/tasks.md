...
- [x] 1.1 Add `tdx-desktop-trading-management` delta spec for ordinary PingAn sell seam routing.
- [x] 1.2 Validate the OpenSpec change before implementation.

## 2. Red Test

- [x] 2.1 Add a focused `TdxTradeManager.pingan.sell(...)` manager test that expects seam delegation.
- [x] 2.2 Run the focused test and confirm it fails because ordinary sell has not delegated to the seam yet.

## 3. Implementation

- [x] 3.1 Route `TdxTradeManager.pingan.sell(...)` through `PingAnExecutionRequest` and `execute_pingan_order`.
