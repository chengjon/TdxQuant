# provider replay status summary lifecycle operation counts tasks

## 1. Specification

- [x] Add an OpenSpec delta requiring read-only `summary_view.status_summary` lifecycle operation count fields.
- [x] Validate the active OpenSpec change in strict mode.

## 2. Red Tests

- [x] Add focused CLI summary-view assertions for the new `status_summary` lifecycle operation count fields.
- [x] Run focused tests and confirm the failure is the missing fields.

## 3. Implementation

- [x] Project the fields from existing lifecycle operation summary in `tdxquant/cli.py`.
