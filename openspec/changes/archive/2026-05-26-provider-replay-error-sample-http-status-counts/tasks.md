...
- [x] 1.1 Add OpenSpec proposal, design, and provider replay delta for error-sample HTTP status counts.
- [x] 1.2 Validate the active OpenSpec change before implementation.

## 2. Tests

- [x] 2.1 Add detailed provider replay assertions for `runtime.probe_summary.error_sample_http_status_counts` and key count.
- [x] 2.2 Add CLI summary-view assertions that projected `probe_summary` includes those fields.

## 3. Implementation

- [x] 3.1 Derive `error_sample_http_status_counts` from existing error sample candidates.
- [x] 3.2 Derive `error_sample_http_status_key_count` from that count map.
