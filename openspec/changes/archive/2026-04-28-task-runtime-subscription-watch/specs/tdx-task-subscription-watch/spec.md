...
- **AND** each normalized row MUST include `schema_version`, `session_id`, `provider_instance_id`, `subscription_id`, `sequence`, `event_type`, `symbol`, `source_ts`, `event_ts`, `reconnect_metadata`, and `payload`
