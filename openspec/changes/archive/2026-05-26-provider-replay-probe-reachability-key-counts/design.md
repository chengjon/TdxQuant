...
Provider replay status is a read-only diagnostic surface for replay HTTP fake provider probes. Probe summary already aggregates reachability maps for requested, healthy, and failed probes; this change adds map-length fields for consumers that need compact shape metadata.

## Goals / Non-Goals

**Goals:**

- Derive `requested_reachability_key_count` from `requested_reachability_counts`.
- Derive `healthy_reachability_key_count` from `healthy_reachability_counts`.
- Derive `failed_reachability_key_count` from `failed_reachability_counts`.
...
- Add the fields in `_build_provider_replay_probe_summary()`.
  - Rationale: the reachability maps are built in the same function, so derived key counts can be computed locally without broadening interfaces.
...
- Risk: readers may treat reachability key counts as endpoint coverage or readiness evidence.
