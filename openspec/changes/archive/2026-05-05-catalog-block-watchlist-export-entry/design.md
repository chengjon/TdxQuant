...
现有 block watchlist export 链路已经具备：
...
因此，这条 change 的工作不是设计新的 catalog 模型，而是把 `export-zxg-watchlist` 接进现有 catalog。

## Goals / Non-Goals

**Goals**

- 正式定义 `export-zxg-watchlist` 为稳定 catalog entry。
...
- 不新增 `catalog show`
- 不新增 inline 参数输入
- 不修改 provider/task export contract
