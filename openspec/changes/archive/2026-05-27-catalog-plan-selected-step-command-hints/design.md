# Design: Catalog Plan Selected Step Command Hints
...
The selected-step summary carries first/last selected step source, name, and entry hints.
It omits first/last dispatch command names even though the non-executing plan payload already resolves them.

## Design

Copy command-name hints from the first and last selected step dispatch objects into `selected_step_summary`:

- `first_step_command_name`
- `last_step_command_name`

If a selected step lacks dispatch command metadata, the field value remains `None`.
