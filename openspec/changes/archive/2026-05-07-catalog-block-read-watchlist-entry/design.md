...
现有 block read watchlist 链路已经具备：
...
因此，这条 change 的工作不是设计新的 catalog 模型，而是把 `read-zxg-watchlist` 接进现有 catalog。

## Goals / Non-Goals

**Goals**

- 正式定义 `read-zxg-watchlist` 为稳定 catalog entry。
...
- 不新增 `catalog show`
- 不新增 inline 参数输入
- 不修改 provider/task `block-read-watchlist` contract
...
- `catalog run --entry read-zxg-watchlist`
  继续等价于：
  - `task run --preset read-zxg-watchlist`
- `catalog plan --entry read-zxg-watchlist`
