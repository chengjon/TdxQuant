...
#### Scenario: D-07 PingAn selected-step boundary presence evidence stays bounded

- **WHEN** D-07 cites PingAn bundle selected-step plan and preview boundary presence evidence
- **THEN** D-07 MUST remain `[部分实现]`
- **AND** the D-07 boundary MUST state that the presence flag is read-only, non-executing catalog summary evidence
