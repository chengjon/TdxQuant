...
| E-07 | wider capability replay coverage | `[部分实现]` | 已有 `51` 个 provider fixture；`market-stock-info-success` / `market-more-info-success` / `market-cb-info-success` / `meta-gb-info-success` / `meta-ipo-info-success` / `meta-gp-one-success` / `meta-divid-factors-success` / `transaction-stock-transaction-data-by-date-success` / `transaction-market-transaction-data-by-date-success` / `transaction-sector-transaction-data-by-date-success` / `transaction-sector-transaction-data-success` 覆盖 `market.stock_info`、`market.more_info`、`market.cb_info`、`meta.gb_info`、`meta.ipo_info`、`meta.gp_one_data`、`meta.divid_factors`、`transaction.stock_transaction_data_by_date`、`transaction.market_transaction_data_by_date`、`transaction.sector_transaction_data_by_date`、`transaction.sector_transaction_data`；对应 `execute_sync_replay(...)`、`TdxApiManager.market/meta/transaction.*(provider_mode="replay")`、`api stock-info/more-info/cb-info/gb-info/ipo-info/gp-one/divid-factors/stock-transaction-data-by-date/market-transaction-data-by-date/sector-transaction-data-by-date/sector-transaction-data --provider-mode replay` 与 `tdx-data-stock-info/more-info/cb-info/gb-info/ipo-info/gp-one/divid-factors/stock-transaction-by-date/market-transaction-by-date/sector-transaction-by-date/sector-transaction --provider-mode replay` 已有 `tests/test_replay_fixtures.py` / `tests/test_replay_provider.py` / `tests/test_api_cli.py` 覆盖；OpenSpec `provider-replay-stock-info-coverage` / `provider-replay-more-info-coverage` / `provider-replay-cb-info-coverage` / `provider-replay-gb-info-coverage` / `provider-replay-ipo-info-coverage` / `provider-replay-gp-one-coverage` / `provider-replay-divid-factors-coverage` / `provider-replay-stock-transaction-by-date-coverage` / `provider-replay-market-transaction-by-date-coverage` / `provider-replay-sector-transaction-by-date-coverage` / `provider-replay-sector-transaction-range-coverage` | 当前新增的是 stock-info、more-info、cb-info、gb-info、ipo-info、gp-one、divid-factors、stock-transaction-data-by-date、market-transaction-data-by-date、sector-transaction-data-by-date、sector-transaction-data 十一个 fixture-backed query 能力；其他未逐项补样例、契约和测试的边缘 capability 仍不承诺 replay 可用。 |
