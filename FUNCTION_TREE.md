...
| E-06 | daemon fake provider | `[部分实现]` | `ProviderTransportReplayHTTPServer` 支持 bearer token、allowlist、watch status/events/SSE fake provider 投影；`build_provider_transport_replay_status` 与 `provider-replay status --config` 暴露 replay-only、foreground、read-only 生命周期边界；`provider-replay status --probe-health --probe-timeout` 可显式只读探测配置中的 `/provider/v1/replay/health` 并返回 `runtime.health_probe`；`subscription-watch-event-stream-delayed-playback.jsonl`；`tests/test_provider_transport_replay.py`、`tests/test_api_cli.py`；OpenSpec `provider-replay-lifecycle-status-boundary` / `provider-replay-status-health-probe` | 当前 fake provider 只覆盖离线只读 replay transport、可查询边界状态和显式 health probe；默认 `status` 不启动 socket、不观察运行态，probe 只观察已配置 HTTP replay service reachable/health，不代表 daemon 管理；没有 start/stop 生命周期控制、真实调度、真实行情会话、restart/backoff 或长期守护进程管理。 |
