...
| ID | 功能节点 | 状态 | 证据 | 边界 |
| --- | --- | --- | --- | --- |
| A-01 | CLI 命令面 | `[已实现]` | `tdxquant/cli.py` 注册约 `194` 个 parser；`tests/test_api_cli.py`、`tests/test_trader_cli.py` | 命令存在不等于每个命令都可在非 Windows 环境完成真实外部调用；外设/TDX 客户端相关命令依赖本机环境。 |
...
| A-03 | Task Manager 统一入口 | `[已实现]` | `tdxquant/api/task.py`：`TdxTaskManager`；`runtime/task-profiles.json` 约 `21` 个 profile；`runtime/task-presets.json` 约 `9` 个 preset | 已有日常任务入口，但更厚的组合任务仍按具体节点登记为待实现。 |
...
| A-06 | command catalog | `[已实现]` | `tdxquant/catalog.py`；`runtime/command-catalog.json` 约 `113` 个 entry；`runtime/TdxQuant_Command_Catalog_Usage.md` | catalog 描述可运行命令和参数编排，不替代能力状态判断；状态以本文件为准。 |
...
| B-10 | block.sync_watchlist | `[已实现]` | `tdxquant/block_sync.py`；provider fixture `block-sync-*.json`；`tests/test_block_sync.py` | 已支持受控同步；覆盖写、增量写的高阶任务入口仍按 E-03 登记为待实现。 |
