...
| D-08 | PingAn submit_once | `[部分实现]` | `TdxTradeManager.pingan.buy_submit_once`；`trade submit-once --side buy/sell` 与 `task trade-submit-once --side buy/sell` 显式暴露 side 边界，默认 buy；`TdxTaskManager.trade_submit_once(side="sell")` 走既有 `pingan.sell` 兼容路由；`tests/test_api_cli.py`、`tests/test_api_manager.py`；OpenSpec `trade-submit-once-side-boundary` | 支持当前适配过的 submit-once 路径；`side=sell` 是 submit_once 入口下的兼容 sell 路由，不是独立 `run_pingan_sell_submit_once` 底层实现；不代表所有交易品种、券商、异常弹窗都已覆盖。 |
