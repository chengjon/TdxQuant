...
| C-02 | task profiles / presets | `[已实现]` | `runtime/task-profiles.json` 约 `21` 个 profile；`runtime/task-presets.json` 约 `12` 个 preset | profiles/presets 是默认参数和组合入口，不保证所有环境一键完成。 |
...
| D-08 | PingAn submit_once | `[部分实现]` | `TdxTradeManager.pingan.buy_submit_once/sell_submit_once`；`trade submit-once --side buy/sell` 与 `task trade-submit-once --side buy/sell` 显式暴露 side 边界，默认 buy；`PingAnDesktopTraderGateway(execution_mode="submit_once", side=sell)` 与 `TdxTaskManager.trade_submit_once(side="sell")` 走 `sell_submit_once` 管理器/审计身份；`audit-daily/period-pingan-sell-submit-once-*` report preset、catalog entry 与 `sell-submit-once-pingan-*-review` bundle 已登记；`tests/test_trade_manager.py`、`tests/test_pingan_trader_gateway.py`、`tests/test_api_cli.py`、`tests/test_api_manager.py`；OpenSpec `trade-submit-once-side-boundary` / `trade-sell-submit-once-entry` / `sell-submit-once-audit-registry` | 支持当前适配过的 buy/sell submit-once 路径、独立方法身份和 PingAn sell_submit_once audit 发现入口；`sell_submit_once` 仍复用既有 `run_pingan_sell_fast` 桌面流，不代表存在独立 `run_pingan_sell_submit_once` 原语；不代表所有交易品种、券商、异常弹窗、结果弹窗或非 PingAn sell_submit_once 报表族都已覆盖。 |
...
| E-11 | 更多 task/report 组合入口 | `[部分实现]` | `runtime/command-bundles.json` 约 `153` 个 bundle，其中约 `99` 个组合 `task` 与 `report` step；代表入口包括 `confirm-complete-review`、`submit-ready-complete-review`、`guarded-buy-complete-review`、`sell-submit-once-pingan-exception-review`、`sell-pingan-exception-review`；`tests/test_api_cli.py` 覆盖 follow-up label discovery、`catalog plan --bundle confirm-complete-review`、sell-submit-once bundle 与普通 sell follow-up bundle 非执行解析；OpenSpec `task-report-combo-entry-registry` / `sell-submit-once-audit-registry` / `trade-sell-catalog-followup-registry` | 当前是固定 runtime JSON preset/bundle 组合入口，不是任意 workflow builder；bundle 仍通过既有 `task` / `report` catalog entry 分发，不新增底层交易、报表或聚合语义，未覆盖的组合继续按具体节点登记；普通 sell follow-up 真实执行仍需显式订单参数和交易安全前置条件。 |
