...
| E-07 | wider capability replay coverage | `[部分实现]` | 已有 `47` 个 provider fixture；`market-stock-info-success` / `market-more-info-success` / `market-cb-info-success` / `meta-gb-info-success` / `meta-ipo-info-success` / `meta-gp-one-success` / `meta-divid-factors-success` 覆盖 `market.stock_info`、`market.more_info`、`market.cb_info`、`meta.gb_info`、`meta.ipo_info`、`meta.gp_one_data`、`meta.divid_factors`；对应 `execute_sync_replay(...)`、`TdxApiManager.market/meta.*(provider_mode="replay")`、`api stock-info/more-info/cb-info/gb-info/ipo-info/gp-one/divid-factors --provider-mode replay` 与 `tdx-data-stock-info/more-info/cb-info/gb-info/ipo-info/gp-one/divid-factors --provider-mode replay` 已有 `tests/test_replay_fixtures.py` / `tests/test_replay_provider.py` / `tests/test_api_cli.py` 覆盖；OpenSpec `provider-replay-stock-info-coverage` / `provider-replay-more-info-coverage` / `provider-replay-cb-info-coverage` / `provider-replay-gb-info-coverage` / `provider-replay-ipo-info-coverage` / `provider-replay-gp-one-coverage` / `provider-replay-divid-factors-coverage` | 当前新增的是 stock-info、more-info、cb-info、gb-info、ipo-info、gp-one、divid-factors 七个 fixture-backed query 能力；`transaction.*_by_date` 等未逐项补样例、契约和测试的边缘 capability 仍不承诺 replay 可用。 |
