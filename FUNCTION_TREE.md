...
| D-08 | PingAn submit_once | `[部分实现]` | `TdxTradeManager.pingan.buy_submit_once/sell_submit_once`；`trade submit-once --side buy/sell` 与 `task trade-submit-once --side buy/sell` 显式暴露 side 边界，默认 buy；`PingAnDesktopTraderGateway(execution_mode="submit_once", side=sell)` 与 `TdxTaskManager.trade_submit_once(side="sell")` 走 `sell_submit_once` 管理器/审计身份；`tests/test_trade_manager.py`、`tests/test_pingan_trader_gateway.py`、`tests/test_api_cli.py`、`tests/test_api_manager.py`；OpenSpec `trade-submit-once-side-boundary` / `trade-sell-submit-once-entry` | 支持当前适配过的 buy/sell submit-once 路径和独立方法身份；`sell_submit_once` 仍复用既有 `run_pingan_sell_fast` 桌面流，不代表存在独立 `run_pingan_sell_submit_once` 原语；不代表所有交易品种、券商、异常弹窗、结果弹窗或 sell submit-once 报表 preset 都已覆盖。 |
