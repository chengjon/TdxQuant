...
| E-11 | 更多 task/report 组合入口 | `[部分实现]` | `runtime/command-bundles.json` 约 `159` 个 bundle，其中约 `105` 个组合 `task` 与 `report` step；代表入口包括 `confirm-complete-review`、`submit-ready-complete-review`、`guarded-buy-complete-review`、`confirm-current-pingan-exception-review`、`buy-submit-once-pingan-exception-review`、`sell-submit-once-pingan-exception-review`、`sell-submit-once-pingan-rejection-review`、`sell-submit-once-pingan-failure-review`；`tests/test_api_cli.py` 覆盖 follow-up label discovery、`catalog plan --bundle confirm-complete-review`、`task-buy-submit-once` / `task-sell-submit-once` catalog list/plan、confirm_current PingAn alias、buy/sell submit-once bundle 与普通 sell follow-up bundle 非执行解析；OpenSpec `task-report-combo-entry-registry` / `sell-submit-once-audit-registry` / `trade-sell-catalog-followup-registry` / `trade-submit-once-sell-catalog-entry` / `confirm-current-pingan-followup-registry` / `trade-submit-once-buy-catalog-entry` | 当前是固定 runtime JSON preset/bundle 组合入口，不是任意 workflow builder；bundle 仍通过既有 `task` / `report` catalog entry 分发，不新增底层交易、报表或聚合语义，未覆盖的组合继续按具体节点登记；普通 buy/sell/confirm_current follow-up 真实执行仍需显式交易安全前置条件。 |
