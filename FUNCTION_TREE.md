...
| D-07 | PingAn buy / sell / confirm_current | `[部分实现]` | `TdxTradeManager.pingan.buy/sell/confirm_current`；稳定 CLI/task 入口包含 `trade buy`、`trade sell`、`task trade-buy`、`task trade-sell`、`trade confirm-current`；`task-sell-default` preset、`task-sell` catalog entry 与 `sell-pingan-*-review` follow-up bundle 已登记；`tdxquant/trade/manager.py`、`tdxquant/api/task.py`；`runtime/trade-audits/*`；`tests/test_trade_manager.py`、`tests/test_api_cli.py`、`tests/test_api_manager.py`；OpenSpec `trade-sell-stable-entry` / `trade-sell-catalog-followup-registry` | 已有买入、卖出、确认链路、稳定 sell 入口、catalog 发现入口和审计；真实交易仍必须显式提供订单参数并受 max_price、submission_key、人工/环境安全约束；不代表所有交易品种、券商、异常弹窗或结果弹窗分支都已覆盖。 |
...
| E-11 | 更多 task/report 组合入口 | `[部分实现]` | `runtime/command-bundles.json` 约 `153` 个 bundle，其中约 `99` 个组合 `task` 与 `report` step；代表入口包括 `confirm-complete-review`、`submit-ready-complete-review`、`guarded-buy-complete-review`、`sell-submit-once-pingan-exception-review`、`sell-submit-once-pingan-rejection-review`、`sell-submit-once-pingan-failure-review`；`tests/test_api_cli.py` 覆盖 follow-up label discovery、`catalog plan --bundle confirm-complete-review`、`task-sell-submit-once` catalog list/plan、sell-submit-once bundle 与普通 sell follow-up bundle 非执行解析；OpenSpec `task-report-combo-entry-registry` / `sell-submit-once-audit-registry` / `trade-sell-catalog-followup-registry` / `trade-submit-once-sell-catalog-entry` | 当前是固定 runtime JSON preset/bundle 组合入口，不是任意 workflow builder；bundle 仍通过既有 `task` / `report` catalog entry 分发，不新增底层交易、报表或聚合语义，未覆盖的组合继续按具体节点登记；普通 sell follow-up 真实执行仍需显式订单参数和交易安全前置条件。 |
