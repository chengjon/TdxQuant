...
| E-09 | 更厚 subscription 长跑包装 | `[部分实现]` | `SubscriptionWatchBackgroundController.status()` 返回 additive `status_summary`，汇总 control/watch_status 的 state、heartbeat、watermark、reconnect/degraded 字段，并可在显式阈值下计算 heartbeat 与 watermark `fresh/stale`、`age_seconds`、`stale_after_seconds`、`evaluated_at`；bridge HTTP/registry/CLI watch-status 已转发 `heartbeat_stale_after_seconds` 与 `watermark_stale_after_seconds`；`tests/test_subscription_watch_background.py`、`tests/test_bridge_http.py`、`tests/test_bridge_registry.py`、`tests/test_api_cli.py`；OpenSpec `subscription-long-run-status-summary` / `subscription-watch-heartbeat-staleness-summary` / `subscription-watch-watermark-staleness-summary` | 当前仍是只读 long-run status projection：保留 raw `control`/`watch_status`，默认 `staleness=not_evaluated`；显式 stale 诊断不改 reconnect/backoff 调度，不改变 start/stop/list/events/logs 或 SSE/event-stream contract。 |
