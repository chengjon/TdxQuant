...
| E-07 | wider capability replay coverage | `[部分实现]` | 已有 `41` 个 provider fixture；`market-stock-info-success` 覆盖 `market.stock_info`；`execute_sync_replay("market.stock_info")`、`TdxApiManager.market.stock_info(provider_mode="replay")`、`api stock-info --provider-mode replay` 与 `tdx-data-stock-info --provider-mode replay` 已有 `tests/test_replay_fixtures.py` / `tests/test_replay_provider.py` / `tests/test_api_cli.py` 覆盖；OpenSpec `provider-replay-stock-info-coverage` | 当前新增的是 stock-info 这一个 fixture-backed query 能力；`market.more_info`、`market.cb_info`、`meta.gb_info`、`meta.ipo_info` 等边缘 capability 仍未承诺 replay 可用，新增能力仍需逐项补样例、契约和测试。 |
