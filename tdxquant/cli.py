...
from .cli_api import build_api_parser as _build_api_parser_impl, handle_api_subcommand as _handle_api_subcommand_impl
from .catalog import (
...
from .cli_catalog import (
    CatalogCommandBoundary,
    build_catalog_parser as _build_catalog_command_parser,
    select_catalog_output_payload as _select_catalog_command_output_payload,
)
from .trade import (
...
def _build_catalog_parser(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> argparse.ArgumentParser:
    return _build_catalog_command_parser(subparsers)
...
def _build_bridge_parser(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> argparse.ArgumentParser:
    ...
    bridge_watch_supervisor_tick_parser = bridge_subparsers.add_parser("watch-supervisor-tick")
...
def _build_api_parser(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> argparse.ArgumentParser:
    return _build_api_parser_impl(subparsers)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    _add_replay_provider_arguments(tdx_data_more_info_parser)
    ...
    _add_replay_provider_arguments(tdx_data_cb_info_parser)
    ...
    _add_replay_provider_arguments(tdx_data_sector_transaction_by_date_parser)
    ...
    _add_replay_provider_arguments(tdx_data_market_transaction_by_date_parser)
...
def _build_catalog_selected_step_summary(summary: dict[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "step_source_name_counts": summary.get("step_source_name_counts"),
        ...
        "trade_plan_boundary_input_kind_counts": copy.deepcopy(
            summary.get("trade_plan_boundary_input_kind_counts", {})
        ),
...
def _build_catalog_plan_summary(summary: dict[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "step_source_name_counts": selected_steps.get("step_source_name_counts"),
        ...
        "trade_plan_boundary_input_kind_counts": copy.deepcopy(
            selected_steps.get("trade_plan_boundary_input_kind_counts", {})
        ),
...
def _build_catalog_step_name_counts(steps: object) -> dict[str, int]:
    counts: dict[str, int] = {}
    if not isinstance(steps, list):
        return counts
    for step in steps:
...
def _build_catalog_step_source_name_counts(steps: object) -> dict[str, int]:
    counts: dict[str, int] = {}
    if not isinstance(steps, list):
        return counts
    for step in steps:
...
def _build_catalog_step_entry_counts(steps: object) -> dict[str, int]:
    counts: dict[str, int] = {}
    if not isinstance(steps, list):
        return counts
    for step in steps:
...
def _build_catalog_step_resolved_arg_key_counts(steps: object) -> dict[str, int]:
    counts: dict[str, int] = {}
    if not isinstance(steps, list):
        return counts
    for step in steps:
        ...
        if not isinstance(resolved_args, dict):
            continue
        for key in _extract_catalog_key_fields(resolved_args):
...
def _build_catalog_step_source_resolved_arg_key_counts(steps: object) -> dict[str, int]:
    counts: dict[str, int] = {}
    if not isinstance(steps, list):
        return counts
    for step in steps:
...
def _build_catalog_trade_plan_boundary_rollup(steps: object) -> dict[str, object]:
    ...
    if isinstance(steps, list):
        for step in steps:
            ...
            input_kind = boundary.get("input_kind")
            if isinstance(input_kind, str) and input_kind:
                input_kind_counts[input_kind] = input_kind_counts.get(input_kind, 0) + 1
    ...
    return {
        ...
        "trade_plan_boundary_input_kind_counts": {
            key: input_kind_counts[key] for key in sorted(input_kind_counts)
        },
...
def _build_catalog_summary_view(args: argparse.Namespace, result: Result) -> dict[str, object] | None:
    ...
    if args.catalog_command == "validate":
        ...
        summary = {
            ...
            "entry_source_key_count": len(validation.get("entry_source_counts") or {}),
            ...
            "bundle_step_name_key_count": len(validation.get("bundle_step_name_counts") or {}),
            "bundle_step_entry_counts": copy.deepcopy(validation.get("bundle_step_entry_counts", {})),
            "bundle_step_entry_key_count": len(validation.get("bundle_step_entry_counts") or {}),
            "bundle_step_source_name_counts": copy.deepcopy(
                validation.get("bundle_step_source_name_counts", {})
            ),
            "bundle_step_source_name_key_count": len(
                validation.get("bundle_step_source_name_counts") or {}
            ),
            ...
            "bundle_step_option_key_count": len(
                validation.get("bundle_step_option_key_counts") or {}
            ),
            "bundle_step_source_option_key_counts": copy.deepcopy(
                validation.get("bundle_step_source_option_key_counts", {})
            ),
            "bundle_step_source_option_key_count": len(
                validation.get("bundle_step_source_option_key_counts") or {}
            ),
            ...
            "task_report_bundle_step_name_key_count": len(
                validation.get("task_report_bundle_step_name_counts") or {}
            ),
            "task_report_bundle_step_source_name_counts": copy.deepcopy(
                validation.get("task_report_bundle_step_source_name_counts", {})
            ),
            "task_report_bundle_step_source_name_key_count": len(
                validation.get("task_report_bundle_step_source_name_counts") or {}
            ),
            ...
            "task_report_bundle_step_entry_key_count": len(
                validation.get("task_report_bundle_step_entry_counts") or {}
            ),
            "task_report_bundle_step_option_key_counts": copy.deepcopy(
                validation.get("task_report_bundle_step_option_key_counts", {})
            ),
            "task_report_bundle_step_option_key_count": len(
                validation.get("task_report_bundle_step_option_key_counts") or {}
            ),
            ...
            "task_report_bundle_step_source_option_key_count": len(
                validation.get("task_report_bundle_step_source_option_key_counts") or {}
            ),
            "task_report_bundle_label_counts": copy.deepcopy(
                validation.get("task_report_bundle_label_counts", {})
            ),
            "task_report_bundle_label_key_count": len(
                validation.get("task_report_bundle_label_counts") or {}
            ),
            ...
            "submit_once_bundle_label_key_count": len(
                validation.get("submit_once_bundle_label_counts") or {}
            ),
            "submit_once_bundle_step_source_counts": copy.deepcopy(
                validation.get("submit_once_bundle_step_source_counts", {})
            ),
            "submit_once_bundle_step_source_key_count": len(
                validation.get("submit_once_bundle_step_source_counts") or {}
            ),
            "submit_once_bundle_step_name_counts": copy.deepcopy(
                validation.get("submit_once_bundle_step_name_counts", {})
            ),
            "submit_once_bundle_step_name_key_count": len(
                validation.get("submit_once_bundle_step_name_counts") or {}
            ),
            ...
            "submit_once_bundle_step_entry_key_count": len(
                validation.get("submit_once_bundle_step_entry_counts") or {}
            ),
            "submit_once_bundle_step_source_entry_counts": copy.deepcopy(
                validation.get("submit_once_bundle_step_source_entry_counts", {})
            ),
            "submit_once_bundle_step_source_entry_key_count": len(
                validation.get("submit_once_bundle_step_source_entry_counts") or {}
            ),
            "submit_once_bundle_step_option_key_counts": copy.deepcopy(
                validation.get("submit_once_bundle_step_option_key_counts", {})
            ),
            "submit_once_bundle_step_option_key_count": len(
                validation.get("submit_once_bundle_step_option_key_counts") or {}
            ),
            ...
            "pingan_bundle_label_key_count": len(validation.get("pingan_bundle_label_counts") or {}),
            "pingan_bundle_step_source_counts": copy.deepcopy(
                validation.get("pingan_bundle_step_source_counts", {})
            ),
            "pingan_bundle_step_source_key_count": len(
                validation.get("pingan_bundle_step_source_counts") or {}
            ),
            "pingan_bundle_step_name_counts": copy.deepcopy(
                validation.get("pingan_bundle_step_name_counts", {})
            ),
            "pingan_bundle_step_name_key_count": len(
                validation.get("pingan_bundle_step_name_counts") or {}
            ),
            ...
            "pingan_bundle_step_entry_key_count": len(
                validation.get("pingan_bundle_step_entry_counts") or {}
            ),
            "pingan_bundle_step_source_entry_counts": copy.deepcopy(
                validation.get("pingan_bundle_step_source_entry_counts", {})
            ),
            "pingan_bundle_step_source_entry_key_count": len(
                validation.get("pingan_bundle_step_source_entry_counts") or {}
            ),
            "pingan_bundle_step_option_key_counts": copy.deepcopy(
                validation.get("pingan_bundle_step_option_key_counts", {})
            ),
            "pingan_bundle_step_option_key_count": len(
                validation.get("pingan_bundle_step_option_key_counts") or {}
            ),
        ...
        summary["catalog_summary"] = {
            "mode": "validate",
            "kind": summary.get("kind"),
            "selected_entry": summary.get("selected_entry"),
            "selected_bundle": summary.get("selected_bundle"),
            "selected_label": selected_label,
            "valid": summary.get("valid"),
            "invalid_count": invalid_count,
            ...
            "bundle_count": bundle_summary["count"],
            "bundle_step_count": bundle_summary["step_count"],
            "label_key_count": label_summary["total_key_count"],
            "source_key_count": source_summary["total_key_count"],
            "has_entries": isinstance(entry_count, int) and entry_count > 0,
            "has_bundles": bundle_summary["has_bundles"],
            "has_invalid_entries": isinstance(invalid_count, int) and invalid_count > 0,
            "has_selected_label": label_summary["has_selected_label"],
        }
        ...
        summary["submit_once_bundle_summary"] = {
            "count": summary.get("submit_once_bundle_count"),
            "step_count": summary.get("submit_once_bundle_step_count"),
            "sample_count": summary.get("submit_once_bundle_sample_count"),
            "sample_limit": summary.get("submit_once_bundle_sample_limit"),
            "sample_truncated": summary.get("submit_once_bundle_sample_truncated"),
            "label_key_count": summary.get("submit_once_bundle_label_key_count"),
            "step_source_key_count": summary.get("submit_once_bundle_step_source_key_count"),
            "step_name_key_count": summary.get("submit_once_bundle_step_name_key_count"),
            "step_source_name_key_count": summary.get("submit_once_bundle_step_source_name_key_count"),
            "step_entry_key_count": summary.get("submit_once_bundle_step_entry_key_count"),
            "step_source_entry_key_count": summary.get("submit_once_bundle_step_source_entry_key_count"),
            "step_option_key_count": summary.get("submit_once_bundle_step_option_key_count"),
            "step_source_option_key_count": summary.get("submit_once_bundle_step_source_option_key_count"),
        }
        summary["pingan_bundle_summary"] = {
            "count": summary.get("pingan_bundle_count"),
            "step_count": summary.get("pingan_bundle_step_count"),
            "sample_count": summary.get("pingan_bundle_sample_count"),
            "sample_limit": summary.get("pingan_bundle_sample_limit"),
            "sample_truncated": summary.get("pingan_bundle_sample_truncated"),
            "label_key_count": summary.get("pingan_bundle_label_key_count"),
            "step_source_key_count": summary.get("pingan_bundle_step_source_key_count"),
            "step_name_key_count": summary.get("pingan_bundle_step_name_key_count"),
            "step_source_name_key_count": summary.get("pingan_bundle_step_source_name_key_count"),
            "step_entry_key_count": summary.get("pingan_bundle_step_entry_key_count"),
            "step_source_entry_key_count": summary.get("pingan_bundle_step_source_entry_key_count"),
            "step_option_key_count": summary.get("pingan_bundle_step_option_key_count"),
            "step_source_option_key_count": summary.get("pingan_bundle_step_source_option_key_count"),
        }
    ...
    if "catalog_bundle" in result.data:
        ...
        if mode in {"plan", "preview"}:
            ...
            step_name_counts = _build_catalog_step_name_counts(steps)
            summary["step_name_counts"] = step_name_counts
            summary["step_name_key_count"] = len(step_name_counts)
            step_source_name_counts = _build_catalog_step_source_name_counts(steps)
            summary["step_source_name_counts"] = step_source_name_counts
            summary["step_source_name_key_count"] = len(step_source_name_counts)
            ...
            step_resolved_arg_key_counts = _build_catalog_step_resolved_arg_key_counts(steps)
            summary["step_resolved_arg_key_counts"] = step_resolved_arg_key_counts
            summary["step_resolved_arg_key_count"] = len(step_resolved_arg_key_counts)
            step_source_resolved_arg_key_counts = _build_catalog_step_source_resolved_arg_key_counts(steps)
            summary["step_source_resolved_arg_key_counts"] = step_source_resolved_arg_key_counts
            summary["step_source_resolved_arg_key_count"] = len(step_source_resolved_arg_key_counts)
            if isinstance(steps, list):
                for step in steps:
                    if not isinstance(step, dict):
                        continue
...
def _select_catalog_output_payload(args: argparse.Namespace, result: Result) -> dict[str, object]:
    return _select_catalog_command_output_payload(args, result)
...
def _run_flat_replay_provider_command(args: argparse.Namespace) -> Result | None:
    ...
    if args.command == "tdx-data-market-transaction-by-date":
...
def _validate_catalog_registry(args: argparse.Namespace) -> Result:
    ...
    if effective_kind in {"bundle", "all"}:
        ...
        for bundle_name in bundle_names:
            try:
                ...
                for step in resolved_bundle["steps"]:
                    ...
                    if isinstance(step_name, str) and step_name:
                        ...
                        if isinstance(source, str) and source:
                            ...
                            bundle_step_source_name_counts[source_name] = (
                                bundle_step_source_name_counts.get(source_name, 0) + 1
                            )
                ...
                if "task" in step_sources and "report" in step_sources:
                    ...
                    for step in resolved_bundle["steps"]:
                        ...
                        if isinstance(step_name, str) and step_name:
                            ...
                            if isinstance(source, str) and source:
                                source_name = f"{source}:{step_name}"
                                task_report_bundle_step_source_name_counts[source_name] = (
                                    task_report_bundle_step_source_name_counts.get(source_name, 0) + 1
                                )
                        step_entry = step.get("entry")
                        if isinstance(step_entry, str) and step_entry:
                        ...
                        if isinstance(step_options, dict):
                            for option_key in step_options:
                                if isinstance(option_key, str) and option_key:
                                    task_report_bundle_step_option_key_counts[option_key] = (
                    ...
                    for step in resolved_bundle["steps"]:
                        ...
                        if isinstance(source, str) and source:
                            submit_once_bundle_step_source_counts[source] = (
                                submit_once_bundle_step_source_counts.get(source, 0) + 1
                            )
                        step_name = step.get("name")
                        if isinstance(step_name, str) and step_name:
                            submit_once_bundle_step_name_counts[step_name] = (
                                submit_once_bundle_step_name_counts.get(step_name, 0) + 1
                            )
                        if isinstance(source, str) and source and isinstance(step_name, str) and step_name:
                            source_name = f"{source}:{step_name}"
                        ...
                        if isinstance(step_options, dict):
                            for option_key in step_options:
                                if isinstance(option_key, str) and option_key:
                                    submit_once_bundle_step_option_key_counts[option_key] = (
                ...
                is_pingan_bundle = (
                    "pingan" in bundle_name
                    or any("pingan" in label for label in bundle_labels)
                    or any("pingan" in entry for entry in step_entries)
                )
                if is_pingan_bundle:
                    ...
                    for step in resolved_bundle["steps"]:
                        ...
                        if isinstance(source, str) and source:
                            pingan_bundle_step_source_counts[source] = (
                                pingan_bundle_step_source_counts.get(source, 0) + 1
                            )
                        step_name = step.get("name")
                        if isinstance(step_name, str) and step_name:
                            pingan_bundle_step_name_counts[step_name] = (
                                pingan_bundle_step_name_counts.get(step_name, 0) + 1
                            )
                        if isinstance(source, str) and source and isinstance(step_name, str) and step_name:
                            source_name = f"{source}:{step_name}"
    ...
    validation = {
        ...
        "bundle_step_source_counts": {
            source: bundle_step_source_counts[source] for source in sorted(bundle_step_source_counts)
        },
        ...
        "task_report_bundle_step_entry_counts": {
            ...
            for entry in sorted(task_report_bundle_step_entry_counts)
        },
        ...
        "submit_once_bundle_step_source_counts": {
            source: submit_once_bundle_step_source_counts[source]
            for source in sorted(submit_once_bundle_step_source_counts)
        },
        "submit_once_bundle_step_name_counts": {
            name: submit_once_bundle_step_name_counts[name]
            for name in sorted(submit_once_bundle_step_name_counts)
        },
        "submit_once_bundle_step_source_name_counts": {
        ...
        "pingan_bundle_step_source_counts": {
            source: pingan_bundle_step_source_counts[source]
            for source in sorted(pingan_bundle_step_source_counts)
        },
        "pingan_bundle_step_name_counts": {
            name: pingan_bundle_step_name_counts[name] for name in sorted(pingan_bundle_step_name_counts)
        },
...
def _handle_api_subcommand(args: argparse.Namespace) -> Result:
    return _handle_api_subcommand_impl(args, manager_factory=TdxApiManager)
...
def _build_catalog_command_boundary() -> CatalogCommandBoundary:
    return CatalogCommandBoundary(
        resolve_entry=resolve_command_catalog_entry,
        list_entries=_list_catalog_entries,
        validate_registry=_validate_catalog_registry,
        plan_entry=_plan_catalog_resolved_entry,
        plan_bundle=_plan_catalog_bundle,
        run_entry=_dispatch_catalog_resolved_entry,
        run_bundle=_run_catalog_bundle,
        build_summary_view=_build_catalog_summary_view,
    )


def _handle_catalog_subcommand(args: argparse.Namespace) -> Result:
    return _build_catalog_command_boundary().handle(args)
...
def _handle_bridge_subcommand(args: argparse.Namespace) -> int:
    try:
        ...
        if args.bridge_command == "watch-supervisor-tick":
            return _emit_bridge_payload(
                run_bridge_watch_supervisor_tick(
                    registry_path=args.registry,
                    worker_id=args.worker,
...
def _build_provider_replay_status_summary_view(status: dict[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "lifecycle": {
            ...
            "supervision_summary": copy.deepcopy(lifecycle.get("supervision_summary")),
        },
