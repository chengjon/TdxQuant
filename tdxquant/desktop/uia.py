...
def run_pingan_sell_fast(
    ...
    if not IS_WINDOWS:
        return Result(
            ok=False,
            code=ErrorCode.UNSUPPORTED_PLATFORM,
            message="pingan sell fast is only available from native Windows Python",
            next_action="Run the sell flow from native Windows Python.",
        )
    ...
    if not window_target["ok"]:
        return Result(
            ok=False,
            code=ErrorCode.WINDOW_NOT_FOUND,
            message="pingan sell fast could not find target window",
            data={"title_keyword": title_keyword, "input": {"code": code, "price": price, "quantity": quantity}, "timing": {"steps": timing_steps}},
        )
    target_window = window_target["window"]
    try:
    ...
    activate_side_result = capture_timed(
        "activate_sell_side",
        lambda: capture("activate_sell_side", _activate_pingan_trade_side(title_keyword, target_window, side_label="卖出", post_delay=0.15)),
    )
    if not activate_side_result.ok:
        return Result(
            ok=False,
            code=activate_side_result.code,
            message="pingan sell fast could not activate the sell page",
            data={"title_keyword": title_keyword, "input": {"code": code, "price": price, "quantity": quantity}, "steps": steps, "timing": {"steps": timing_steps}},
            next_action=activate_side_result.next_action,
        )
    ...
    set_code_result = capture_timed("set_code", lambda: capture("set_code", set_cached_text("set_code", "12005", code)))
    if not set_code_result.ok:
        return Result(
            ok=False,
            code=set_code_result.code,
            message="pingan sell fast aborted while setting code",
            data={"title_keyword": title_keyword, "input": {"code": code, "price": price, "quantity": quantity}, "steps": steps, "timing": {"steps": timing_steps}},
        )
    ...
    if not set_price_result.ok:
        return Result(
            ok=False,
            code=set_price_result.code,
            message="pingan sell fast aborted while setting price",
            data={"title_keyword": title_keyword, "input": {"code": code, "price": price, "quantity": quantity}, "steps": steps, "timing": {"steps": timing_steps}},
        )
    ...
    if not set_quantity_result.ok:
        return Result(
            ok=False,
            code=set_quantity_result.code,
            message="pingan sell fast aborted while setting quantity",
            data={"title_keyword": title_keyword, "input": {"code": code, "price": price, "quantity": quantity}, "steps": steps, "timing": {"steps": timing_steps}},
        )
    ...
    if not focus_result.ok:
        return Result(
            ok=False,
            code=focus_result.code,
            message="pingan sell fast aborted before HID submit",
            data={"title_keyword": title_keyword, "input": {"code": code, "price": price, "quantity": quantity}, "steps": steps, "timing": {"steps": timing_steps}},
            next_action="Confirm the quantity input is visible and focusable, then retry.",
        )
    ...
    if not hid_enter_result.ok:
        return Result(
            ok=False,
            code=hid_enter_result.code,
            message="pingan sell fast aborted during HID submit",
            data={"title_keyword": title_keyword, "input": {"code": code, "price": price, "quantity": quantity}, "steps": steps, "timing": {"steps": timing_steps}},
        )
    ...
    def find_confirm_target() -> dict[str, Any]:
        if dialog_lookup_mode == "win32_experimental":
            experimental = _find_pingan_side_confirm_button_win32(timeout=confirm_timeout, side_label="卖出")
            if experimental["ok"]:
                experimental["lookup_mode"] = "win32_experimental"
                return experimental
            fallback = _find_pingan_side_confirm_button(title_keyword=title_keyword, timeout=confirm_timeout, side_label="卖出")
            fallback["lookup_mode"] = "uia_fallback"
            fallback["lookup_fallback_from"] = experimental.get("last_error")
            return fallback
        target = _find_pingan_side_confirm_button(title_keyword=title_keyword, timeout=confirm_timeout, side_label="卖出")
        target["lookup_mode"] = "uia"
        return target
    ...
    if not confirm_target["ok"]:
        return Result(
            ok=False,
            code=ErrorCode.CONTROL_NOT_FOUND,
            message="could not find current pingan sell confirm button after fast submit",
            data={
                "title_keyword": title_keyword,
                "input": {"code": code, "price": price, "quantity": quantity},
                "steps": steps,
            ...
            next_action="Keep the sell confirm dialog visible and inspect the latest UIA tree.",
        )
    ...
    def find_result_dialog() -> dict[str, Any]:
        if dialog_lookup_mode == "win32_experimental":
            target = _find_pingan_result_dialog_win32(timeout=result_timeout)
            if target["ok"]:
                target["lookup_mode"] = "win32_experimental"
                return target
            fallback = _find_pingan_result_dialog(title_keyword=title_keyword, timeout=result_timeout)
            fallback["lookup_mode"] = "uia_fallback"
            fallback["lookup_fallback_from"] = target.get("last_error")
            return fallback
        target = _find_pingan_result_dialog(title_keyword=title_keyword, timeout=result_timeout)
        target["lookup_mode"] = "uia"
        return target
    ...
    return Result(
        ok=ok,
        code=ErrorCode.OK if ok else confirm_click.code,
        message="completed pingan sell fast" if ok else "pingan sell fast encountered an error",
        data={
            "title_keyword": title_keyword,
            "input": {"code": code, "price": price, "quantity": quantity},
            "hid": {"port": port, "baudrate": baudrate, "timeout": timeout},
...
def _find_pingan_trade_side_target_in_window(window: Any, *, side_label: str) -> dict[str, Any]:
    try:
    ...
    for element in candidates:
        try:
            ...
            continue
        if side_label not in name:
            continue
        if "确认" in name or "交易确认" in name:
            continue
        score = 0
        if name == f"{side_label}下单":
            score += 20
        elif name == side_label:
            score += 16
        elif name.startswith(side_label):
            score += 12
        else:
            score += 8
        ...
        if handle:
            score += 1
        scored.append((score, {"ok": True, "window": window, "element": element, "info": info}))

    if not scored:
        return {"ok": False, "last_error": f"{side_label} side target not found"}
    scored.sort(key=lambda item: item[0], reverse=True)
    return scored[0][1]


def _activate_pingan_trade_side(title_keyword: str, window: Any, *, side_label: str, post_delay: float = 0.2) -> Result:
    target = _find_pingan_trade_side_target_in_window(window, side_label=side_label)
    if not target.get("ok"):
        return Result(
            ok=False,
            code=ErrorCode.CONTROL_NOT_FOUND,
            message=f"could not find pingan {side_label} side activation target",
            data={"title_keyword": title_keyword, "side_label": side_label, "last_error": target.get("last_error")},
            next_action=f"Switch to the PingAn {side_label}委托 page and retry.",
        )
    return _activate_target(title_keyword, target, post_delay, "auto")
...
def _find_pingan_side_confirm_button(title_keyword: str, timeout: float, *, side_label: str) -> dict[str, Any]:
    dialog_name = f"{side_label}交易确认"
    button_name = f"{side_label}确认"
    deadline = time.time() + timeout
    last_error: str | None = None
    while time.time() < deadline:
        ...
        for node in nodes:
            if node.get("name") == dialog_name and node.get("class_name") == "#32770":
...
def _find_pingan_confirm_button(title_keyword: str, timeout: float) -> dict[str, Any]:
    return _find_pingan_side_confirm_button(title_keyword=title_keyword, timeout=timeout, side_label="买入")
...
def _find_pingan_side_confirm_button_win32(timeout: float, *, side_label: str) -> dict[str, Any]:
    dialog_name = f"{side_label}交易确认"
    button_name = f"{side_label}确认"
    deadline = time.time() + timeout
    last_error: str | None = None
    while time.time() < deadline:
        dialog = _find_win32_top_window_exact(dialog_name)
        if dialog["ok"]:
            button = _find_win32_child_window(int(dialog["hwnd"]), text=button_name, class_name="Button")
            if button["ok"]:
...
def _find_pingan_confirm_button_win32(timeout: float) -> dict[str, Any]:
    return _find_pingan_side_confirm_button_win32(timeout=timeout, side_label="买入")
