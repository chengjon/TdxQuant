...
class ApiCliParserTests(unittest.TestCase):
    ...
    def test_task_pingan_promotion_readiness_rollup_command_parses(self) -> None:
        ...
        args = parser.parse_args(
            [
                ...
                "3600",
                "--json-output-path",
                "runtime/pingan/promotion-readiness-rollup.json",
            ]
        )
        self.assertEqual(args.command, "task")
    ...
    def test_task_guarded_trade_buy_command_parses(self) -> None:
        ...
        self.assertEqual(args.lifecycle_statefile_path, "/tmp/pingan-owner.json")
        self.assertEqual(args.lifecycle_owner_token, "guarded-owner")
        self.assertEqual(args.lifecycle_stale_after_seconds, 42.5)
        self.assertTrue(args.require_lifecycle_owner_lock)
    ...
    def test_build_trader_service_forwards_submit_once_broker_readiness_guard_to_gateway(self) -> None:
        parser = build_parser()
        args = parser.parse_args(
            [
                "trade",
    ...
    def test_trade_lifecycle_owner_lock_command_parses(self) -> None:
        ...
        self.assertEqual(args.command, "trade")
        self.assertEqual(args.trade_command, "lifecycle-owner-lock")
        self.assertEqual(args.action, "acquire")
...
class ApiCliDispatchTests(unittest.TestCase):
    ...
    def test_handle_catalog_plan_task_report_combo_summary_counts_step_sources(self) -> None:
        ...
        self.assertEqual(
            output_payload["selected_step_summary"],
            {
                ...
                "trade_plan_boundary_input_coverage_status_counts": {"no_required_inputs": 1},
                "trade_plan_boundary_input_kind_counts": {"confirmation": 1},
    ...
    def test_handle_catalog_submit_once_bundle_exposes_trade_boundary_coverage_status_counts(self) -> None:
        parser = build_parser()
        cases = [
            (
                ["catalog", "plan", "--bundle", "buy-submit-once-pingan-complete-review", "--view", "summary"],
                {"missing_required_inputs": 1},
                True,
                ["buy"],
            ),
            (
                ["catalog", "preview", "--bundle", "sell-submit-once-pingan-complete-review", "--view", "summary"],
                {"missing_required_inputs": 1},
                True,
                ["sell"],
            ),
        ...
        for argv, expected_counts, expected_presence, expected_sides in cases:
            with self.subTest(argv=argv):
                ...
                self.assertTrue(result.ok)
                self.assertEqual(
                    output_payload["trade_plan_boundary_input_coverage_status_counts"],
                    expected_counts,
                )
                self.assertEqual(
                    output_payload["selected_step_summary"]["trade_plan_boundary_input_coverage_status_counts"],
                    expected_counts,
                )
                self.assertEqual(
                    output_payload["plan_summary"]["trade_plan_boundary_input_coverage_status_counts"],
                    expected_counts,
                )
                ...
                self.assertEqual(output_payload["trade_plan_boundary_sides"], expected_sides)
                self.assertEqual(output_payload["plan_summary"]["trade_plan_boundary_sides"], expected_sides)
                self.assertEqual(output_payload["constraints"]["dispatch_executed"], False)
                mocked_dispatch.assert_not_called()

    def test_handle_catalog_submit_once_bundle_exposes_trade_boundary_input_kind_counts(self) -> None:
        parser = build_parser()
        cases = [
            (
                ["catalog", "plan", "--bundle", "buy-submit-once-pingan-complete-review", "--view", "summary"],
        ...
        for argv, expected_counts, expected_presence in cases:
            with self.subTest(argv=argv):
                args = parser.parse_args(argv)
                with patch("tdxquant.cli._dispatch_catalog_resolved_entry") as mocked_dispatch:
                    result = _handle_catalog_subcommand(args)
                ...
                self.assertTrue(result.ok)
                self.assertEqual(
                    output_payload["trade_plan_boundary_input_kind_counts"],
                    expected_counts,
                )
                self.assertEqual(
                    output_payload["selected_step_summary"]["trade_plan_boundary_input_kind_counts"],
                    expected_counts,
                )
                self.assertEqual(
                    output_payload["plan_summary"]["trade_plan_boundary_input_kind_counts"],
                    expected_counts,
                )
                self.assertEqual(output_payload["has_trade_plan_boundary"], expected_presence)
                self.assertEqual(output_payload["constraints"]["dispatch_executed"], False)
    ...
    def test_handle_catalog_pingan_bundle_plan_and_preview_expose_trade_command_rollup(self) -> None:
        parser = build_parser()
        cases = [
            ("buy-pingan-complete-review", "trade-buy", "task-buy"),
            ("sell-pingan-complete-review", "trade-sell", "task-sell"),
            (
        ...
        for bundle_name, expected_command, expected_entry in cases:
            for catalog_command in ("plan", "preview"):
                with self.subTest(bundle_name=bundle_name, catalog_command=catalog_command):
                    args = parser.parse_args(
                        [
                    ...
                    self.assertTrue(result.ok)
                    self.assertEqual(output_payload["trade_plan_boundary_step_count"], 1)
                    self.assertEqual(output_payload["trade_plan_boundary_commands"], [expected_command])
                    self.assertEqual(
                        output_payload["selected_step_summary"]["trade_plan_boundary_commands"],
                        [expected_command],
                    )
                    self.assertEqual(
                        output_payload["plan_summary"]["trade_plan_boundary_commands"],
                        [expected_command],
                    )
                    first_step = output_payload["steps"][0]
                    self.assertEqual(first_step["entry"], expected_entry)
    ...
    def test_handle_catalog_pingan_bundle_exposes_trade_boundary_coverage_status_counts(self) -> None:
        parser = build_parser()
        cases = [
            (
                ["catalog", "plan", "--bundle", "buy-pingan-complete-review", "--view", "summary"],
                {"missing_required_inputs": 1},
                True,
            ),
            (
                ["catalog", "preview", "--bundle", "buy-pingan-complete-review", "--view", "summary"],
                {"missing_required_inputs": 1},
                True,
            ),
            (
                ["catalog", "plan", "--bundle", "confirm-current-pingan-complete-review", "--view", "summary"],
                {"no_required_inputs": 1},
                True,
            ),
            (
                ["catalog", "plan", "--bundle", "buy-pingan-complete-review", "--from-step", "success", "--view", "summary"],
    ...
    def test_handle_catalog_pingan_bundle_exposes_trade_boundary_input_kind_counts(self) -> None:
        parser = build_parser()
        cases = [
            (
                ["catalog", "plan", "--bundle", "buy-pingan-complete-review", "--view", "summary"],
            ...
            (
                ["catalog", "preview", "--bundle", "sell-pingan-complete-review", "--view", "summary"],
                {"order": 1},
                True,
            ),
            (
...
class TaskCliDispatchTests(unittest.TestCase):
    ...
    def test_handle_task_trade_submit_once_forwards_broker_readiness_guard(self) -> None:
        parser = build_parser()
        args = parser.parse_args(
            [
                "task",
        ...
        self.assertIs(result, expected)
        self.assertEqual(manager.trade_submit_once.call_args.kwargs["side"], "sell")
        self.assertTrue(manager.trade_submit_once.call_args.kwargs["require_broker_readiness"])
    ...
    def test_handle_task_guarded_trade_buy_forwards_lifecycle_owner_lock_guard(self) -> None:
        parser = build_parser()
        args = parser.parse_args(
            [
                "task",
        ...
        self.assertIs(result, expected)
        call = manager.guarded_trade_buy.call_args.kwargs
        self.assertEqual(call["lifecycle_statefile_path"], "/tmp/pingan-owner.json")
...
class TradeCliDispatchTests(unittest.TestCase):
    ...
    def test_run_trade_lifecycle_owner_lock_forwards_owner_lock_arguments(self) -> None:
        ...
        expected = Result(ok=True, code=ErrorCode.OK, message="ok", data={"lifecycle_owner_lock": {"status": "released"}})
        manager = MagicMock()
        manager.pingan.lifecycle_owner_lock.return_value = expected
        with patch("tdxquant.cli.TdxTradeManager", return_value=manager):
            result = _run_trade_lifecycle_owner_lock(args)
        self.assertIs(result, expected)
        manager.pingan.lifecycle_owner_lock.assert_called_once_with(
            action="release",
            statefile_path="runtime/pingan/lifecycle-owner.json",
