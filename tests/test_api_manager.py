...
class ApiContextTests(unittest.TestCase):
    ...
    def test_runtime_report_presets_include_confirm_trade_audit_status_presets(self) -> None:
        presets = load_report_presets()
        expected = {
            "audit-daily-confirm-rejected": "rejected",
            "audit-period-confirm-rejected": "rejected",
            "audit-daily-confirm-failed": "failed",
            "audit-period-confirm-failed": "failed",
        }
        for preset, status in expected.items():
            with self.subTest(preset=preset):
                self.assertIn(preset, presets)
                self.assertEqual(presets[preset]["options"]["method"], "confirm_current")
                self.assertEqual(presets[preset]["options"]["status"], status)

    def test_runtime_report_presets_include_pingan_confirm_trade_audit_status_presets(self) -> None:
        presets = load_report_presets()
        expected = {
            "audit-daily-pingan-confirm-rejected": "rejected",
            "audit-period-pingan-confirm-rejected": "rejected",
            "audit-daily-pingan-confirm-failed": "failed",
            "audit-period-pingan-confirm-failed": "failed",
        }
        for preset, status in expected.items():
            with self.subTest(preset=preset):
                self.assertIn(preset, presets)
                self.assertEqual(presets[preset]["options"]["broker"], "pingan")
                self.assertEqual(presets[preset]["options"]["method"], "confirm_current")
                self.assertEqual(presets[preset]["options"]["status"], status)
    ...
    def test_runtime_report_presets_include_submit_once_trade_audit_status_presets(self) -> None:
        presets = load_report_presets()
        expected = {
            "audit-daily-submit-once-rejected": "rejected",
            "audit-period-submit-once-rejected": "rejected",
            "audit-daily-submit-once-failed": "failed",
            "audit-period-submit-once-failed": "failed",
        }
        for preset, status in expected.items():
            with self.subTest(preset=preset):
                self.assertIn(preset, presets)
                self.assertEqual(presets[preset]["options"]["method"], "buy_submit_once")
                self.assertEqual(presets[preset]["options"]["status"], status)
    ...
    def test_runtime_report_presets_include_pingan_submit_once_trade_audit_status_presets(self) -> None:
        presets = load_report_presets()
        expected = {
            "audit-daily-pingan-submit-once-rejected": "rejected",
            "audit-period-pingan-submit-once-rejected": "rejected",
            "audit-daily-pingan-submit-once-failed": "failed",
            "audit-period-pingan-submit-once-failed": "failed",
        }
        for preset, status in expected.items():
            with self.subTest(preset=preset):
                self.assertIn(preset, presets)
    ...
    def test_runtime_report_presets_include_pingan_sell_submit_once_trade_audit_presets(self) -> None:
        presets = load_report_presets()
        self.assertIn("audit-daily-pingan-sell-submit-once-exceptions", presets)
        self.assertIn("audit-period-pingan-sell-submit-once-exceptions", presets)
        self.assertEqual(presets["audit-daily-pingan-sell-submit-once-exceptions"]["options"]["broker"], "pingan")
        self.assertEqual(
            presets["audit-daily-pingan-sell-submit-once-exceptions"]["options"]["method"],
            "sell_submit_once",
        )
        self.assertEqual(
            presets["audit-daily-pingan-sell-submit-once-exceptions"]["options"]["statuses"],
            ["rejected", "failed"],
        )
        expected = {
            "audit-daily-pingan-sell-submit-once-rejected": "rejected",
            "audit-period-pingan-sell-submit-once-rejected": "rejected",
            "audit-daily-pingan-sell-submit-once-failed": "failed",
            "audit-period-pingan-sell-submit-once-failed": "failed",
        }
        for preset, status in expected.items():
            with self.subTest(preset=preset):
                self.assertIn(preset, presets)
                self.assertEqual(presets[preset]["options"]["broker"], "pingan")
                self.assertEqual(presets[preset]["options"]["method"], "sell_submit_once")
                self.assertEqual(presets[preset]["options"]["status"], status)
    ...
    def test_runtime_report_presets_include_buy_trade_audit_status_presets(self) -> None:
        presets = load_report_presets()
        expected = {
            "audit-daily-buy-rejected": "rejected",
            "audit-period-buy-rejected": "rejected",
            "audit-daily-buy-failed": "failed",
            "audit-period-buy-failed": "failed",
        }
        for preset, status in expected.items():
            with self.subTest(preset=preset):
                self.assertIn(preset, presets)
                self.assertEqual(presets[preset]["options"]["method"], "buy")
                self.assertEqual(presets[preset]["options"]["status"], status)
    ...
    def test_runtime_report_presets_include_pingan_buy_trade_audit_status_presets(self) -> None:
        presets = load_report_presets()
        expected = {
            "audit-daily-pingan-buy-rejected": "rejected",
            "audit-period-pingan-buy-rejected": "rejected",
            "audit-daily-pingan-buy-failed": "failed",
            "audit-period-pingan-buy-failed": "failed",
        }
        for preset, status in expected.items():
            with self.subTest(preset=preset):
                self.assertIn(preset, presets)
    ...
    def test_runtime_report_presets_include_sell_trade_audit_status_presets(self) -> None:
        presets = load_report_presets()
        expected = {
            "audit-daily-sell-rejected": "rejected",
            "audit-period-sell-rejected": "rejected",
            "audit-daily-sell-failed": "failed",
            "audit-period-sell-failed": "failed",
        }
        for preset, status in expected.items():
            with self.subTest(preset=preset):
                self.assertIn(preset, presets)
                self.assertEqual(presets[preset]["options"]["method"], "sell")
                self.assertEqual(presets[preset]["options"]["status"], status)
    ...
    def test_runtime_report_presets_include_pingan_sell_trade_audit_status_presets(self) -> None:
        presets = load_report_presets()
        expected = {
            "audit-daily-pingan-sell-rejected": "rejected",
            "audit-period-pingan-sell-rejected": "rejected",
            "audit-daily-pingan-sell-failed": "failed",
            "audit-period-pingan-sell-failed": "failed",
        }
        for preset, status in expected.items():
            with self.subTest(preset=preset):
                self.assertIn(preset, presets)
    ...
    def test_runtime_report_presets_include_submit_path_trade_audit_status_presets(self) -> None:
        presets = load_report_presets()
        expected = {
            "audit-daily-submit-path-rejected": "rejected",
            "audit-period-submit-path-rejected": "rejected",
            "audit-daily-submit-path-failed": "failed",
            "audit-period-submit-path-failed": "failed",
        }
        for preset, status in expected.items():
            with self.subTest(preset=preset):
                self.assertIn(preset, presets)
                self.assertEqual(
                    presets[preset]["options"]["methods"],
                    ["buy_submit_once", "confirm_current"],
                )
                self.assertEqual(presets[preset]["options"]["status"], status)
    ...
    def test_runtime_report_presets_include_pingan_submit_path_trade_audit_status_presets(self) -> None:
        presets = load_report_presets()
        expected = {
            "audit-daily-pingan-submit-path-rejected": "rejected",
            "audit-period-pingan-submit-path-rejected": "rejected",
            "audit-daily-pingan-submit-path-failed": "failed",
            "audit-period-pingan-submit-path-failed": "failed",
        }
        for preset, status in expected.items():
            with self.subTest(preset=preset):
                self.assertIn(preset, presets)
    ...
    def test_runtime_report_presets_include_order_single_status_presets(self) -> None:
        presets = load_report_presets()
        expected = {
            "audit-daily-order-rejected": "rejected",
            "audit-period-order-rejected": "rejected",
            "audit-daily-order-failed": "failed",
            "audit-period-order-failed": "failed",
        }
        for preset, status in expected.items():
            with self.subTest(preset=preset):
                self.assertIn(preset, presets)
    ...
    def test_runtime_command_catalog_includes_confirm_trade_audit_status_entries(self) -> None:
        entries = load_command_catalog()
        expected = [
            "audit-daily-confirm-rejected",
            "audit-period-confirm-rejected",
            "audit-daily-confirm-failed",
            "audit-period-confirm-failed",
        ]
    ...
    def test_runtime_command_catalog_includes_pingan_confirm_trade_audit_status_entries(self) -> None:
        entries = load_command_catalog()
        expected = [
            "audit-daily-pingan-confirm-rejected",
            "audit-period-pingan-confirm-rejected",
            "audit-daily-pingan-confirm-failed",
            "audit-period-pingan-confirm-failed",
        ]
        for entry in expected:
            with self.subTest(entry=entry):
                self.assertIn(entry, entries)
    ...
    def test_runtime_command_catalog_includes_submit_once_trade_audit_status_entries(self) -> None:
        entries = load_command_catalog()
        expected = [
            "audit-daily-submit-once-rejected",
            "audit-period-submit-once-rejected",
            "audit-daily-submit-once-failed",
            "audit-period-submit-once-failed",
        ]
        for entry in expected:
            with self.subTest(entry=entry):
                self.assertIn(entry, entries)
    ...
    def test_runtime_command_catalog_includes_pingan_submit_once_trade_audit_status_entries(self) -> None:
        entries = load_command_catalog()
        expected = [
            "audit-daily-pingan-submit-once-rejected",
            "audit-period-pingan-submit-once-rejected",
            "audit-daily-pingan-submit-once-failed",
            "audit-period-pingan-submit-once-failed",
        ]
        for entry in expected:
            with self.subTest(entry=entry):
                self.assertIn(entry, entries)
    ...
    def test_runtime_command_catalog_includes_pingan_sell_submit_once_trade_audit_entries(self) -> None:
        entries = load_command_catalog()
        expected = [
            "audit-daily-pingan-sell-submit-once-exceptions",
            "audit-period-pingan-sell-submit-once-exceptions",
            "audit-daily-pingan-sell-submit-once-rejected",
            "audit-period-pingan-sell-submit-once-rejected",
            "audit-daily-pingan-sell-submit-once-failed",
            "audit-period-pingan-sell-submit-once-failed",
        ]
        for entry in expected:
            with self.subTest(entry=entry):
                self.assertIn(entry, entries)
                self.assertEqual(entries[entry]["preset"], entry)
                self.assertIn("sell-submit-once", entries[entry]["labels"])
    ...
    def test_runtime_command_catalog_includes_buy_trade_audit_status_entries(self) -> None:
        entries = load_command_catalog()
        expected = [
            "audit-daily-buy-rejected",
            "audit-period-buy-rejected",
            "audit-daily-buy-failed",
            "audit-period-buy-failed",
        ]
        for entry in expected:
            with self.subTest(entry=entry):
                self.assertIn(entry, entries)
    ...
    def test_runtime_command_catalog_includes_pingan_buy_trade_audit_status_entries(self) -> None:
        entries = load_command_catalog()
        expected = [
            "audit-daily-pingan-buy-rejected",
            "audit-period-pingan-buy-rejected",
            "audit-daily-pingan-buy-failed",
            "audit-period-pingan-buy-failed",
        ]
        for entry in expected:
            with self.subTest(entry=entry):
                self.assertIn(entry, entries)
    ...
    def test_runtime_command_catalog_includes_sell_trade_audit_status_entries(self) -> None:
        entries = load_command_catalog()
        expected = [
            "audit-daily-sell-rejected",
            "audit-period-sell-rejected",
            "audit-daily-sell-failed",
            "audit-period-sell-failed",
        ]
        for entry in expected:
            with self.subTest(entry=entry):
                self.assertIn(entry, entries)
    ...
    def test_runtime_command_catalog_includes_pingan_sell_trade_audit_status_entries(self) -> None:
        entries = load_command_catalog()
        expected = [
            "audit-daily-pingan-sell-rejected",
            "audit-period-pingan-sell-rejected",
            "audit-daily-pingan-sell-failed",
            "audit-period-pingan-sell-failed",
        ]
        for entry in expected:
            with self.subTest(entry=entry):
                self.assertIn(entry, entries)
    ...
    def test_runtime_command_catalog_includes_submit_path_trade_audit_status_entries(self) -> None:
        entries = load_command_catalog()
        expected = [
            "audit-daily-submit-path-rejected",
            "audit-period-submit-path-rejected",
            "audit-daily-submit-path-failed",
            "audit-period-submit-path-failed",
        ]
        for entry in expected:
            with self.subTest(entry=entry):
                self.assertIn(entry, entries)
    ...
    def test_runtime_command_catalog_includes_pingan_submit_path_trade_audit_status_entries(self) -> None:
        entries = load_command_catalog()
        expected = [
            "audit-daily-pingan-submit-path-rejected",
            "audit-period-pingan-submit-path-rejected",
            "audit-daily-pingan-submit-path-failed",
            "audit-period-pingan-submit-path-failed",
        ]
        for entry in expected:
            with self.subTest(entry=entry):
                self.assertIn(entry, entries)
    ...
    def test_runtime_command_catalog_includes_order_single_status_entries(self) -> None:
        entries = load_command_catalog()
        expected = [
    ...
    def test_runtime_command_bundles_include_confirm_trade_audit_status_bundles(self) -> None:
        bundles = load_command_bundles()
        expected = {
            "audit-confirm-rejection-diagnostics": "audit-daily-confirm-rejected",
            "audit-confirm-failure-diagnostics": "audit-daily-confirm-failed",
            "confirm-rejection-review": "audit-daily-confirm-rejected",
            "confirm-failure-review": "audit-daily-confirm-failed",
        }
    ...
    def test_runtime_command_bundles_include_pingan_confirm_trade_audit_status_bundles(self) -> None:
        bundles = load_command_bundles()
        expected = {
            "audit-pingan-confirm-rejection-diagnostics": "audit-daily-pingan-confirm-rejected",
            "audit-pingan-confirm-failure-diagnostics": "audit-daily-pingan-confirm-failed",
            "confirm-pingan-rejection-review": "audit-daily-pingan-confirm-rejected",
            "confirm-pingan-failure-review": "audit-daily-pingan-confirm-failed",
        }
        for bundle, audit_entry in expected.items():
            with self.subTest(bundle=bundle):
                self.assertIn(bundle, bundles)
    ...
    def test_runtime_command_bundles_include_submit_once_trade_audit_status_bundles(self) -> None:
        bundles = load_command_bundles()
        expected = {
            "audit-submit-once-rejection-diagnostics": "audit-daily-submit-once-rejected",
            "audit-submit-once-failure-diagnostics": "audit-daily-submit-once-failed",
            "submit-once-rejection-review": "audit-daily-submit-once-rejected",
            "submit-once-failure-review": "audit-daily-submit-once-failed",
        }
        for bundle, audit_entry in expected.items():
            with self.subTest(bundle=bundle):
                self.assertIn(bundle, bundles)
    ...
    def test_runtime_command_bundles_include_pingan_submit_once_trade_audit_status_bundles(self) -> None:
        bundles = load_command_bundles()
        expected = {
            "audit-pingan-submit-once-rejection-diagnostics": "audit-daily-pingan-submit-once-rejected",
            "audit-pingan-submit-once-failure-diagnostics": "audit-daily-pingan-submit-once-failed",
            "submit-once-pingan-rejection-review": "audit-daily-pingan-submit-once-rejected",
            "submit-once-pingan-failure-review": "audit-daily-pingan-submit-once-failed",
        }
        for bundle, audit_entry in expected.items():
            with self.subTest(bundle=bundle):
                self.assertIn(bundle, bundles)
    ...
    def test_runtime_command_bundles_include_pingan_sell_submit_once_trade_audit_bundles(self) -> None:
        bundles = load_command_bundles()
        expected = {
            "audit-pingan-sell-submit-once-exception-diagnostics": "audit-daily-pingan-sell-submit-once-exceptions",
            "audit-pingan-sell-submit-once-rejection-diagnostics": "audit-daily-pingan-sell-submit-once-rejected",
            "audit-pingan-sell-submit-once-failure-diagnostics": "audit-daily-pingan-sell-submit-once-failed",
            "sell-submit-once-pingan-exception-review": "audit-daily-pingan-sell-submit-once-exceptions",
            "sell-submit-once-pingan-rejection-review": "audit-daily-pingan-sell-submit-once-rejected",
            "sell-submit-once-pingan-failure-review": "audit-daily-pingan-sell-submit-once-failed",
        }
        for bundle, audit_entry in expected.items():
            with self.subTest(bundle=bundle):
                self.assertIn(bundle, bundles)
    ...
    def test_runtime_command_bundles_include_buy_trade_audit_status_bundles(self) -> None:
        bundles = load_command_bundles()
        expected = {
            "audit-buy-rejection-diagnostics": "audit-daily-buy-rejected",
            "audit-buy-failure-diagnostics": "audit-daily-buy-failed",
            "guarded-buy-rejection-review": "audit-daily-buy-rejected",
            "guarded-buy-failure-review": "audit-daily-buy-failed",
        }
        for bundle, audit_entry in expected.items():
            with self.subTest(bundle=bundle):
                self.assertIn(bundle, bundles)
    ...
    def test_runtime_command_bundles_include_pingan_buy_trade_audit_status_bundles(self) -> None:
        bundles = load_command_bundles()
        expected = {
            "audit-pingan-buy-rejection-diagnostics": "audit-daily-pingan-buy-rejected",
            "audit-pingan-buy-failure-diagnostics": "audit-daily-pingan-buy-failed",
            "guarded-pingan-buy-rejection-review": "audit-daily-pingan-buy-rejected",
            "guarded-pingan-buy-failure-review": "audit-daily-pingan-buy-failed",
        }
        for bundle, audit_entry in expected.items():
            with self.subTest(bundle=bundle):
                self.assertIn(bundle, bundles)
    ...
    def test_runtime_command_bundles_include_sell_trade_audit_status_bundles(self) -> None:
        bundles = load_command_bundles()
        expected = {
            "audit-sell-rejection-diagnostics": "audit-daily-sell-rejected",
            "audit-sell-failure-diagnostics": "audit-daily-sell-failed",
        }
        for bundle, audit_entry in expected.items():
            with self.subTest(bundle=bundle):
                self.assertIn(bundle, bundles)
    ...
    def test_runtime_command_bundles_include_pingan_sell_trade_audit_status_bundles(self) -> None:
        bundles = load_command_bundles()
        expected = {
            "audit-pingan-sell-rejection-diagnostics": "audit-daily-pingan-sell-rejected",
            "audit-pingan-sell-failure-diagnostics": "audit-daily-pingan-sell-failed",
        }
        for bundle, audit_entry in expected.items():
            with self.subTest(bundle=bundle):
                self.assertIn(bundle, bundles)
    ...
    def test_runtime_command_bundles_include_submit_path_trade_audit_status_bundles(self) -> None:
        bundles = load_command_bundles()
        expected = {
            "audit-submit-path-rejection-diagnostics": "audit-daily-submit-path-rejected",
            "audit-submit-path-failure-diagnostics": "audit-daily-submit-path-failed",
            "confirm-submit-path-rejection-review": "audit-daily-submit-path-rejected",
            "confirm-submit-path-failure-review": "audit-daily-submit-path-failed",
        }
        for bundle, audit_entry in expected.items():
            with self.subTest(bundle=bundle):
                self.assertIn(bundle, bundles)
    ...
    def test_runtime_command_bundles_include_pingan_submit_path_trade_audit_status_bundles(self) -> None:
        bundles = load_command_bundles()
        expected = {
            "audit-pingan-submit-path-rejection-diagnostics": "audit-daily-pingan-submit-path-rejected",
            "audit-pingan-submit-path-failure-diagnostics": "audit-daily-pingan-submit-path-failed",
            "confirm-pingan-submit-path-rejection-review": "audit-daily-pingan-submit-path-rejected",
            "confirm-pingan-submit-path-failure-review": "audit-daily-pingan-submit-path-failed",
        }
        for bundle, audit_entry in expected.items():
            with self.subTest(bundle=bundle):
                self.assertIn(bundle, bundles)
    ...
    def test_runtime_command_bundles_include_order_single_status_bundles(self) -> None:
        bundles = load_command_bundles()
        expected = {
            "audit-order-rejection-diagnostics": "audit-daily-order-rejected",
            "audit-order-failure-diagnostics": "audit-daily-order-failed",
        }
        for bundle, audit_entry in expected.items():
            with self.subTest(bundle=bundle):
                self.assertIn(bundle, bundles)
    ...
    def test_runtime_command_bundles_include_submit_ready_trade_audit_status_followups(self) -> None:
        bundles = load_command_bundles()
        expected = {
            "submit-ready-rejection-review": "audit-daily-rejected",
            "submit-ready-failure-review": "audit-daily-failed",
            "submit-ready-pingan-rejection-review": "audit-daily-pingan-rejected",
            "submit-ready-pingan-failure-review": "audit-daily-pingan-failed",
        }
        for bundle, audit_entry in expected.items():
            with self.subTest(bundle=bundle):
                self.assertIn(bundle, bundles)
    ...
    def test_runtime_command_bundles_include_submit_ready_submit_path_exception_followups(self) -> None:
        bundles = load_command_bundles()
        expected = {
            "submit-ready-submit-path-exception-review": "audit-daily-submit-path-exceptions",
            "submit-ready-pingan-submit-path-exception-review": "audit-daily-pingan-submit-path-exceptions",
        }
        for bundle, audit_entry in expected.items():
            with self.subTest(bundle=bundle):
                self.assertIn(bundle, bundles)
    ...
    def test_runtime_command_bundles_include_submit_ready_submit_path_status_followups(self) -> None:
        bundles = load_command_bundles()
        expected = {
            "submit-ready-submit-path-rejection-review": "audit-daily-submit-path-rejected",
            "submit-ready-submit-path-failure-review": "audit-daily-submit-path-failed",
            "submit-ready-pingan-submit-path-rejection-review": "audit-daily-pingan-submit-path-rejected",
            "submit-ready-pingan-submit-path-failure-review": "audit-daily-pingan-submit-path-failed",
        }
        for bundle, audit_entry in expected.items():
            with self.subTest(bundle=bundle):
                self.assertIn(bundle, bundles)
    ...
    def test_runtime_command_bundles_include_submit_once_order_exception_followups(self) -> None:
        bundles = load_command_bundles()
        expected = {
            "submit-once-order-exception-review": "audit-daily-order-exceptions",
            "submit-once-pingan-order-exception-review": "audit-daily-pingan-order-exceptions",
        }
        for bundle, audit_entry in expected.items():
            with self.subTest(bundle=bundle):
                self.assertIn(bundle, bundles)
    ...
    def test_runtime_command_bundles_include_submit_once_order_status_followups(self) -> None:
        bundles = load_command_bundles()
        expected = {
            "submit-once-order-rejection-review": "audit-daily-order-rejected",
            "submit-once-order-failure-review": "audit-daily-order-failed",
            "submit-once-pingan-order-rejection-review": "audit-daily-pingan-order-rejected",
            "submit-once-pingan-order-failure-review": "audit-daily-pingan-order-failed",
        }
        for bundle, audit_entry in expected.items():
            with self.subTest(bundle=bundle):
                self.assertIn(bundle, bundles)
    ...
    def test_runtime_command_bundles_include_confirm_order_exception_followups(self) -> None:
        bundles = load_command_bundles()
        expected = {
            "confirm-order-exception-review": "audit-daily-order-exceptions",
            "confirm-pingan-order-exception-review": "audit-daily-pingan-order-exceptions",
        }
        for bundle, audit_entry in expected.items():
            with self.subTest(bundle=bundle):
                self.assertIn(bundle, bundles)
    ...
    def test_runtime_command_bundles_include_confirm_order_status_followups(self) -> None:
        bundles = load_command_bundles()
        expected = {
            "confirm-order-rejection-review": "audit-daily-order-rejected",
            "confirm-order-failure-review": "audit-daily-order-failed",
            "confirm-pingan-order-rejection-review": "audit-daily-pingan-order-rejected",
            "confirm-pingan-order-failure-review": "audit-daily-pingan-order-failed",
        }
        for bundle, audit_entry in expected.items():
            with self.subTest(bundle=bundle):
                self.assertIn(bundle, bundles)
    ...
    def test_runtime_command_bundles_include_guarded_buy_order_status_followups(self) -> None:
        bundles = load_command_bundles()
        expected = {
            "guarded-buy-order-rejection-review": "audit-daily-order-rejected",
            "guarded-buy-order-failure-review": "audit-daily-order-failed",
            "guarded-pingan-buy-order-rejection-review": "audit-daily-pingan-order-rejected",
            "guarded-pingan-buy-order-failure-review": "audit-daily-pingan-order-failed",
        }
        for bundle, audit_entry in expected.items():
            with self.subTest(bundle=bundle):
                self.assertIn(bundle, bundles)
...
class TdxTaskManagerTests(unittest.TestCase):
    ...
    @staticmethod
    def _pingan_ready_review_packet() -> dict:
        return {
            "schema": "tdx.desktop_trade.pingan_implemented_status_review_packet.v1",
    ...
    @classmethod
    def _pingan_blocked_review_packet(cls) -> dict:
        packet = cls._pingan_ready_review_packet()
        packet.update(
            {
                "review_status": "blocked",
                "decision": "blocked",
                "implemented_status_eligible": False,
                "completed_gates": ["provider_broker_ownership", "audit_evidence"],
                "incomplete_gates": ["safety_gates", "desktop_lifecycle"],
                "blocked_reasons": ["incomplete_required_gates"],
            }
        )
        return packet
    ...
    def test_task_pingan_implemented_status_review_result_writes_controlled_artifact(self) -> None:
        with TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            packet_path = root / "implemented-status-review-packet.json"
            output_path = root / "implemented-status-review-result.json"
            packet_path.write_text(json.dumps(self._pingan_ready_review_packet()), encoding="utf-8")
            manager = TdxTaskManager(profile="default", strategy_path="strategy.py")

            result = manager.pingan_implemented_status_review_result(
        ...
        self.assertEqual(artifact["schema"], "tdx.desktop_trade.pingan_implemented_status_review_result.v1")
        self.assertEqual(artifact["artifact_provenance"]["source_kind"], "implemented_status_review_result")
        self.assertEqual(artifact["artifact_provenance"]["producer"], "task pingan-implemented-status-review-result")
        self.assertEqual(
            artifact["artifact_provenance"]["evidence_schema"],
            "tdx.desktop_trade.pingan_implemented_status_review_result.v1",
        )
        self.assertEqual(artifact["reviewer"], "maintainer-a")
        self.assertEqual(artifact["outcome"], "approve")
        self.assertEqual(artifact["reviewed_at"], "2026-06-01T10:00:00Z")
        self.assertEqual(artifact["target_nodes"], ["D-07", "D-08"])
        self.assertEqual(artifact["packet_review_status"], "ready_for_manual_review")
        self.assertEqual(artifact["packet_decision"], "eligible_for_review")
        self.assertTrue(artifact["implemented_status_eligible"])
        self.assertFalse(artifact["function_tree_status_transition_executed"])
        self.assertFalse(artifact["automatic_status_transition_allowed"])
        self.assertFalse(artifact["order_submitted"])
        self.assertFalse(artifact["control_dispatch_executed"])
        self.assertEqual(artifact["execution_mode"], "manual_status_review_result_record")
        self.assertEqual(artifact["side_effect_level"], "file_write")
        record = result.data["review_result_record"]
        self.assertTrue(record["artifact_written"])
        self.assertEqual(record["outcome"], "approve")
        self.assertEqual(record["packet_review_status"], "ready_for_manual_review")
        self.assertFalse(record["function_tree_status_transition_executed"])

    def test_task_pingan_implemented_status_review_result_dry_run_does_not_write(self) -> None:
        with TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            packet_path = root / "promotion-readiness-rollup.json"
            output_path = root / "implemented-status-review-result.json"
            packet_path.write_text(
                json.dumps(
                    {
                        "promotion_readiness_rollup": {
                            "implemented_status_review_packet": self._pingan_ready_review_packet()
                        }
                    }
                ),
                encoding="utf-8",
            ...
            result = manager.pingan_implemented_status_review_result(
                ...
                reviewer="maintainer-a",
                outcome="defer",
                reason="waiting for separate status transition review",
                reviewed_at="2026-06-01T10:05:00Z",
                dry_run=True,
            )
        ...
        artifact = result.data["implemented_status_review_result"]
        self.assertEqual(artifact["outcome"], "defer")
        self.assertEqual(artifact["side_effect_level"], "none")
        record = result.data["review_result_record"]
        self.assertFalse(record["artifact_written"])
        self.assertTrue(record["dry_run"])
        self.assertEqual(record["side_effect_level"], "none")

    def test_task_pingan_implemented_status_review_result_rejects_approve_for_blocked_packet(self) -> None:
        with TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            packet_path = root / "implemented-status-review-packet.json"
            output_path = root / "implemented-status-review-result.json"
            packet_path.write_text(json.dumps(self._pingan_blocked_review_packet()), encoding="utf-8")
            manager = TdxTaskManager(profile="default", strategy_path="strategy.py")
            ...
            result = manager.pingan_implemented_status_review_result(
                ...
                reviewer="maintainer-a",
                outcome="approve",
                reason="attempted approval should fail closed",
                reviewed_at="2026-06-01T10:10:00Z",
            )
    ...
    def test_task_pingan_promotion_readiness_rollup_blocks_schema_less_complete_evidence(self) -> None:
        with TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            preflight_path = root / "preflight.json"
            dialog_path = root / "dialog-readiness.json"
        ...
        self.assertEqual(
            contract["invalid_source_kinds"],
            ["preflight", "dialog_readiness", "acceptance_coverage"],
        )
        self.assertFalse(contract["source_statuses"]["preflight"]["schema_valid"])
        self.assertFalse(contract["source_statuses"]["dialog_readiness"]["schema_valid"])
        self.assertFalse(contract["source_statuses"]["acceptance_coverage"]["schema_valid"])
        decision = rollup["implemented_status_promotion_decision"]
        self.assertEqual(decision["decision"], "blocked")
        self.assertFalse(decision["implemented_status_eligible"])
    ...
    def test_task_trade_submit_once_forwards_broker_readiness_guard_to_buy_side(self) -> None:
        trade_result = Result(ok=True, code=ErrorCode.OK, message="ok", data={"artifacts": {}, "result_dialog": {}})
        manager = TdxTaskManager(profile="trade_submit_once", strategy_path="strategy.py")
        with patch.object(type(manager.trade_manager.pingan), "buy_submit_once", return_value=trade_result) as mocked_trade:
            result = manager.trade_submit_once(
    ...
    def test_task_trade_submit_once_forwards_broker_readiness_guard_to_sell_side(self) -> None:
        trade_result = Result(ok=True, code=ErrorCode.OK, message="ok", data={"artifacts": {}, "result_dialog": {}})
        manager = TdxTaskManager(profile="trade_submit_once", strategy_path="strategy.py")
        with patch.object(type(manager.trade_manager.pingan), "sell_submit_once", return_value=trade_result) as mocked_trade:
            result = manager.trade_submit_once(
    ...
    def test_task_trade_submit_once_sell_side_uses_sell_submit_once_identity(self) -> None:
        trade_result = Result(
            ...
            data={
                "artifacts": {},
                "result_dialog": {"contract_no": "S202604260004"},
            },
        )
        manager = TdxTaskManager(profile="trade_submit_once", strategy_path="strategy.py")
        with (
        ...
        self.assertTrue(result.ok)
        self.assertEqual(result.data["task"]["name"], "trade_submit_once")
        self.assertEqual(result.data["input"]["side"], "sell")
        self.assertEqual(result.data["result_dialog"]["contract_no"], "S202604260004")
