...
class TdxTradeManagerTests(unittest.TestCase):
    ...
    def test_pingan_buy_submit_once_routes_through_execution_module(self) -> None:
        with TemporaryDirectory() as temp_dir:
            manager = TdxTradeManager(
                profile="submit_once",
                state_path=str(Path(temp_dir) / "state.json"),
            ...
            with (
                patch("tdxquant.trade.manager.execute_pingan_order", return_value=delegated) as mocked_execute,
                patch("tdxquant.trade.manager.run_pingan_buy_submit_once") as mocked_desktop,
            ):
                result = manager.pingan.buy_submit_once(
                    port="COM3",
                    code="000001",
                    price="10.00",
                    quantity=100,
                    submission_key="submit-once-routing-001",
                    max_price=10.50,
                )
        ...
        mocked_execute.assert_called_once()
        request = mocked_execute.call_args.args[0]
        self.assertEqual(request.method, "buy_submit_once")
        self.assertEqual(request.timing_label, "pingan.buy_submit_once")
        self.assertEqual(request.request_context(), {"code": "000001", "price": "10.00", "quantity": 100})
        self.assertEqual(mocked_execute.call_args.kwargs["idempotency"]["decision"], "execute")
        self.assertTrue(mocked_execute.call_args.kwargs["risk_gate"]["passed"])
        mocked_desktop.assert_not_called()
    ...
    def test_pingan_sell_routes_through_execution_module(self) -> None:
        with TemporaryDirectory() as temp_dir:
            manager = TdxTradeManager(
                profile="balanced",
                state_path=str(Path(temp_dir) / "state.json"),
            ...
            with (
                ...
                result = manager.pingan.sell(
                    ...
                    price="10.00",
                    quantity=100,
                    submission_key="sell-routing-001",
                    max_price=10.50,
                )
        ...
        mocked_execute.assert_called_once()
        request = mocked_execute.call_args.args[0]
        self.assertEqual(request.method, "sell")
        self.assertEqual(request.timing_label, "pingan.sell")
        self.assertEqual(request.request_context(), {"code": "000001", "price": "10.00", "quantity": 100})
        self.assertEqual(mocked_execute.call_args.kwargs["idempotency"]["decision"], "execute")
        self.assertTrue(mocked_execute.call_args.kwargs["risk_gate"]["passed"])
        mocked_desktop.assert_not_called()
