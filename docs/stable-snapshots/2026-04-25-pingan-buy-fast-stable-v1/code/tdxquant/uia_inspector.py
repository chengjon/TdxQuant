...
def run_pingan_buy_fast(
    ...
    confirm_click = capture_timed(
        "confirm_click_wm_command",
        lambda: capture("confirm_click_wm_command", _click_runtime_target(confirm_target, strategy="wm_command", post_delay=confirm_post_delay)),
    )
    result_dialog = capture_timed("result_dialog_lookup", lambda: _find_pingan_result_dialog(title_keyword=title_keyword, timeout=result_timeout))
    detected_result_dialog_data: dict[str, Any] = {}
    if result_dialog["ok"]:
        ...
        if close_result_dialog:
            ...
            if result_confirm_target["ok"]:
                capture_timed(
                    "result_dialog_focus_confirm",
                    lambda: capture("result_dialog_focus_confirm", _focus_runtime_target(result_confirm_target, post_delay=max(0.05, result_close_pre_delay))),
                )
            capture_timed(
                "result_dialog_close",
                lambda: capture(
