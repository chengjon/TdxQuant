# Task Block Read Watchlist Preset Design
...
第一版 `task-block-read-watchlist-preset` 只是在现有 preset 体系里新增一个允许目标命令，并不引入新的 profile 概念或第二套 preset 数据模型。

## Context

`task block-read-watchlist` 已经落地为稳定的读侧快照 task，解决了：

- provider-level `block.read_watchlist_snapshot(...)`
- task-level `data.snapshot`
- 独立 CLI 入口：
  - `tdxquant task block-read-watchlist --block-code ZXG`

但它还不能像 `block-read-full` 和 `block-read-watchlist-export` 一样，通过现有 task preset 体系复用固定的日常参数。当前缺的不是新的 preset 机制，而是把这条 task 接进已经存在的：
...
- 让 `block-read-watchlist` 成为现有 `task run --preset ...` 的合法目标命令

## Goals

- 把 `block-read-watchlist` 接入现有 task preset 体系
- 继续复用：
  - `runtime/task-presets.json`
  - `resolve_task_preset(...)`
  - `task run --preset ...`
...
- 不把 `block-read-watchlist` 接进 `TASK_COMMAND_DEFAULT_PROFILES` 之外的第二套 preset 路径

## Recommended Approach

采用最小接入方案：

- 只把 `block-read-watchlist` 加入 task preset allowlist
...
```json
{
  "read-zxg-watchlist": {
    ...
    "description": "Read normalized watchlist snapshot for ZXG.",
    "profile": "default",
    "options": {
      "block_code": "ZXG"
    }
...
- 但这条 change 不扩 `block-read-watchlist` 的 task-specific 参数面
...
`task presets` 应能够列出指向 `block-read-watchlist` 的 preset。

### Execution

`task run --preset read-zxg-watchlist` 最终仍然应 dispatch 到：
...
`block-read-watchlist` 继续复用 `_add_task_common_arguments(...)`，因此通用 `--output` 仍存在；它只表示“把整条 JSON result 写到文件”，不是这条 preset 线的领域参数。
...
| 函数 / 常量 | 改动? | 原因 |
|---|---|---|
| `TASK_COMMAND_DEFAULT_PROFILES` | yes | 新增 `"block-read-watchlist": "default"` 以把它纳入 preset allowlist |
| `resolve_task_preset(...)` | no | 现有职责只是 load + normalize + override merge，不需要为这条命令新增特殊逻辑 |
| `_build_task_preset_namespace(...)` | yes | 只补 `block-read-watchlist` 缺少 `block_code` 的显式早失败，避免把配置错误延后到更深层 dispatch |
| `_handle_task_subcommand(...)` | no | `block-read-watchlist` 的 dispatch 分支已经存在，只需让 preset 合并后的 namespace 正确到达它 |
...
- preset 解析后能把 `task_command` 设置为 `block-read-watchlist`
- preset `options.block_code` 能正确映射到 dispatch 读取的 `args.block_code`
- `task run --preset ... --block-code ...` 能覆盖 preset 默认值
- `block-read-watchlist` preset 若缺少 `block_code`，必须在 preset execution 阶段稳定失败
- 最终复用现有 dispatch：
  - `manager.block_read_watchlist(...)`

这里要明确：

- `block-read-watchlist` 的独立 dispatch 分支已经存在
...
- `task run --preset ...` 能正确 dispatch 到 `manager.block_read_watchlist(...)`
...
- listing 包含 `block-read-watchlist`
- `task run --preset ...` 走到 `manager.block_read_watchlist(...)`
...
因为这些已经由 `task block-read-watchlist` 或其他既有功能覆盖。

## Risks / Trade-offs

- [把 preset 接入做成第二套解析系统]
  - 通过复用现有 `_build_task_preset_namespace(...)` 来规避
- [把 `block-read-watchlist` 错误扩展成导出型 preset]
...
- [preset 配置错误被延后到更深层 dispatch]
  - 通过对缺少 `block_code` 的 `block-read-watchlist` preset 增加显式早失败来规避
...
- 现有 task preset 体系
- 新增 `block-read-watchlist` 作为允许目标命令
- 静态 `block_code` 默认值
